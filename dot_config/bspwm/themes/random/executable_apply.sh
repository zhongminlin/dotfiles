...
random_theme() {
	...
	if [[ `which wal` ]]; then
		...
		if [[ -f "$WALFILE" ]]; then
			cat "$WALFILE" > "$BDIR"/themes/"$THEME"/theme.bash
			cat "$BDIR"/themes/random/config.bash >> "$BDIR"/themes/"$THEME"/theme.bash
			echo -e "\n# Don't Delete This File" >> "$BDIR"/themes/"$THEME"/theme.bash
